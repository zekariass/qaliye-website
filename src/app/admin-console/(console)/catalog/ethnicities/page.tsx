...
function CreateForm({ onSubmit, onCancel, isLoading }: { onSubmit: (v: CreateValues) => void; onCancel: () => void; isLoading: boolean }) {
  ...
  return (
    <div className="bg-white border border-[#E5E5EA] rounded-xl p-5 space-y-3 mb-4">
      <h3 className="text-sm font-semibold text-[#17171B]">Add Ethnicity</h3>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Name *</label>
          <input value={name} onChange={(e) => setName(e.target.value)} className={INPUT} placeholder="e.g. Oromo" />
        </div>
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Code *</label>
          <input value={code} onChange={(e) => setCode(e.target.value.toLowerCase())} className={`${INPUT} font-mono`} placeholder="e.g. oromo" />
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Region</label>
          <input value={region} onChange={(e) => setRegion(e.target.value)} className={INPUT} placeholder="e.g. Oromia" />
        </div>
        <div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button type="button" onClick={() => onSubmit({ code: code.trim(), countryCode: countryCode.trim().toUpperCase(), name: name.trim(), region: region.trim() || undefined, sortOrder: Number(sortOrder) })} disabled={!valid || isLoading} className="px-3 py-1.5 text-sm font-medium text-white bg-[#7C3AED] rounded-lg disabled:opacity-50">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6 mx-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-[#17171B]">Edit Ethnicity</h2>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-[#17171B] mb-1.5">Region</label>
            <input value={region} onChange={(e) => setRegion(e.target.value)} className={INPUT} placeholder="e.g. Oromia" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="button" onClick={() => onSubmit({ name: name.trim(), region: region.trim() || undefined, isActive, sortOrder: Number(sortOrder) })} disabled={!name.trim() || isLoading} className="px-4 py-2 text-sm font-medium text-white bg-[#7C3AED] hover:bg-[#6D28D9] rounded-lg disabled:opacity-50">
...
export default function EthnicitiesPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery<{ ethnicities?: CatalogItem[]; items?: CatalogItem[] }>({
    queryKey: adminKeys.catalog.ethnicities(),
    queryFn: async () => { const res = await fetch("/api/internal-admin/catalog/ethnicities"); if (!res.ok) throw new Error(`${res.status}`); return res.json(); },
  });

  const items: CatalogItem[] = data?.ethnicities ?? data?.items ?? [];

  const createMutation = useMutation({
    mutationFn: async (body: CreateValues) => {
      const res = await fetch("/api/internal-admin/catalog/ethnicities", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error ?? "Failed to create"); }
    },
    onSuccess: () => { toast.success("Ethnicity added"); queryClient.invalidateQueries({ queryKey: adminKeys.catalog.ethnicities() }); setShowCreate(false); },
    onError: (e: Error) => toast.error(e.message),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, body }: { id: string; body: EditValues }) => {
      const res = await fetch(`/api/internal-admin/catalog/ethnicities/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error ?? "Failed to update"); }
    },
    onSuccess: () => { toast.success("Ethnicity updated"); queryClient.invalidateQueries({ queryKey: adminKeys.catalog.ethnicities() }); setEditing(null); },
    onError: (e: Error) => toast.error(e.message),
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/internal-admin/catalog/ethnicities/${id}`, { method: "DELETE" });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error ?? "Failed to delete"); }
    },
    onSuccess: () => { toast.success("Ethnicity deactivated"); queryClient.invalidateQueries({ queryKey: adminKeys.catalog.ethnicities() }); setDeleting(null); },
    onError: (e: Error) => toast.error(e.message),
  });

  return (
    <div>
      <PageHeader title="Ethnicities" description="Manage supported ethnicities in the catalog" actions={
      ...
      {isLoading ? (
        <div className="space-y-2">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-12 bg-[#E5E5EA] rounded-xl animate-pulse" />)}</div>
      ) : isError ? (
        <ErrorState onRetry={refetch} />
      ) : items.length === 0 ? (
        <EmptyState icon={<UsersRound className="h-10 w-10" />} title="No ethnicities yet" description="Add your first ethnicity to the catalog." />
      ) : (
          ...
          <table className="w-full min-w-max text-sm">
            <thead>
              <tr className="bg-[#F7F7FA] border-b border-[#E5E5EA]">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-[#666672] uppercase tracking-wider">Region</th>
            ...
            <tbody className="divide-y divide-[#E5E5EA]">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-[#F7F7FA]">
                  ...
                  <td className="px-4 py-3 text-[#666672]">{item.region ?? "—"}</td>
