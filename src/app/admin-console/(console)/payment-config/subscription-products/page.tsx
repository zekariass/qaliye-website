...
import { Plus, Pencil, Trash2, Package, X, Eye } from "lucide-react";
...
function CreateForm({ onSubmit, onCancel, isLoading }: { onSubmit: (v: CreateValues) => void; onCancel: () => void; isLoading: boolean }) {
  ...
  const [autoRenewSupported, setAutoRenewSupported] = useState(true);
  const [includedCredits, setIncludedCredits] = useState("0");
  const [isActive, setIsActive] = useState(true);

  const valid = planId.trim() && productCode.trim();

  return (
    <div className="bg-white border border-[#E5E5EA] rounded-xl p-5 space-y-3 mb-4">
      <h3 className="text-sm font-semibold text-[#17171B]">Add Subscription Product</h3>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Plan ID (UUID) *</label>
          <input value={planId} onChange={(e) => setPlanId(e.target.value)} className={`${INPUT} font-mono`} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" />
        </div>
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Product Code *</label>
          <input value={productCode} onChange={(e) => setProductCode(e.target.value)} className={`${INPUT} font-mono`} placeholder="e.g. com.app.gold.monthly" />
        </div>
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Billing Interval Unit</label>
        ...
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Interval Count</label>
          <input value={billingIntervalCount} onChange={(e) => setBillingIntervalCount(e.target.value)} type="number" min="1" className={INPUT} />
        </div>
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Included Credits</label>
          <input value={includedCredits} onChange={(e) => setIncludedCredits(e.target.value)} type="number" min="0" className={INPUT} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-[#17171B] mb-1">Auto-Renew Supported</label>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          type="button"
          onClick={() => onSubmit({ planId: planId.trim(), productCode: productCode.trim(), billingIntervalUnit, billingIntervalCount: Number(billingIntervalCount), autoRenewSupported, includedCredits: Number(includedCredits), isActive })}
...
export default function SubscriptionProductsPage() {
  ...
  const [editing, setEditing] = useState<SubscriptionProduct | null>(null);
  const [deleting, setDeleting] = useState<SubscriptionProduct | null>(null);

  const { data, isLoading, isError, refetch } = useQuery<{ items?: SubscriptionProduct[]; products?: SubscriptionProduct[] }>({
    queryKey: adminKeys.paymentConfig.subscriptionProducts(),
    queryFn: async () => {
      const res = await fetch("/api/internal-admin/payment-config/subscription-products");
      if (!res.ok) throw new Error(`${res.status}`);
      return res.json();
    },
  });

  const items: SubscriptionProduct[] = data?.items ?? data?.products ?? [];

  const createMutation = useMutation({
    mutationFn: async (body: CreateValues) => {
      const res = await fetch("/api/internal-admin/payment-config/subscription-products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error ?? "Failed to create"); }
    },
    onSuccess: () => { toast.success("Subscription product created"); queryClient.invalidateQueries({ queryKey: adminKeys.paymentConfig.subscriptionProducts() }); setShowCreate(false); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Subscription product updated"); queryClient.invalidateQueries({ queryKey: adminKeys.paymentConfig.subscriptionProducts() }); setEditing(null); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Subscription product deactivated"); queryClient.invalidateQueries({ queryKey: adminKeys.paymentConfig.subscriptionProducts() }); setDeleting(null); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Manage store subscription products linked to plans"
        actions={
      ...
      {isLoading ? (
        ...
        <EmptyState icon={<Package className="h-10 w-10" />} title="No subscription products yet" description="Add your first subscription product to get started." />
      ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-[#F7F7FA]">
                  <td className="px-4 py-3 font-mono text-xs text-[#17171B]">
                    <Link href={`${adminConsolePath}/payment-config/subscription-products/${item.id}`} className="hover:text-[#7C3AED] hover:underline">
                      {item.productCode}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5 justify-end">
                      <Link href={`${adminConsolePath}/payment-config/subscription-products/${item.id}`} className="p-1.5 text-[#666672] hover:text-[#7C3AED] hover:bg-[#EDE2FF] rounded-lg" title="View details"><Eye className="h-3.5 w-3.5" /></Link>
      ...
      {deleting && (
        <ConfirmDialog
          ...
          description={`Deactivate "${deleting.productCode}"? It will no longer be available for purchase.`}
