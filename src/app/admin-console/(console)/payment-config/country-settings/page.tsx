...
import { Plus, Pencil, Trash2, Globe, X, Eye } from "lucide-react";
            ...
            <tbody className="divide-y divide-[#E5E5EA]">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-[#F7F7FA]">
                  <td className="px-4 py-3 font-mono text-sm font-semibold text-[#17171B]">
                    <Link href={`${adminConsolePath}/payment-config/country-settings/${item.id}`} className="hover:text-[#7C3AED] hover:underline">
                      {item.countryCode}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5 justify-end">
                      <Link href={`${adminConsolePath}/payment-config/country-settings/${item.id}`} className="p-1.5 text-[#666672] hover:text-[#7C3AED] hover:bg-[#EDE2FF] rounded-lg" title="View details"><Eye className="h-3.5 w-3.5" /></Link>
