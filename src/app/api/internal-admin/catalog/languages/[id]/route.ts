...
export async function PATCH(
  ...
  try {
    ...
    const result = await adminPatch<Record<string, unknown>>(`/catalog/languages/${id}`, body, { requiredRole: "ADMIN" });
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await adminDelete(`/catalog/languages/${id}`, { requiredRole: "ADMIN" });
