...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await adminGet("/payment-config/country-settings", { requiredRole: "ADMIN", params });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await adminPost("/payment-config/country-settings", body, { requiredRole: "ADMIN" });
