...
import { adaptCountrySetting } from "@/lib/admin/adapters";
...
export async function GET(
  ...
  try {
    ...
    const data = await adminGet<Record<string, unknown>>(`/payment-config/country-settings/${id}`, { requiredRole: "ADMIN" });
    return NextResponse.json(adaptCountrySetting(data));
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    const data = await adminPut<Record<string, unknown>>(`/payment-config/country-settings/${id}`, body, { requiredRole: "ADMIN" });
    return NextResponse.json(adaptCountrySetting(data));
  } catch (error) {
    if (error instanceof AdminAuthError) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    if (error instanceof AdminForbiddenError) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await adminDelete(`/payment-config/country-settings/${id}`, { requiredRole: "ADMIN" });
