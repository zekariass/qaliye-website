...
export async function GET(request: NextRequest) {
  try {
    ...
    const raw = await adminGet("/payment-config/feature-actions", { requiredRole: "ADMIN", params });
    return NextResponse.json({ items: extractItems(raw).map(adaptFeatureAction) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await adminPost<Record<string, unknown>>("/payment-config/feature-actions", body, { requiredRole: "ADMIN" });
    return NextResponse.json(adaptFeatureAction(data), { status: 201 });
  } catch (error) {
    if (error instanceof AdminAuthError) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    if (error instanceof AdminForbiddenError) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    if (error instanceof AdminApiError) return NextResponse.json({ error: error.message }, { status: error.status });
