...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await adminGet("/payment-config/feature-actions", { requiredRole: "ADMIN", params });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await adminPost("/payment-config/feature-actions", body, { requiredRole: "ADMIN" });
