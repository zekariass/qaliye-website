...
import { adaptPaymentOffer } from "@/lib/admin/adapters";
...
export async function GET(
  ...
  try {
    ...
    const data = await adminGet<Record<string, unknown>>(`/payment-config/payment-offers/${id}`, { requiredRole: "ADMIN" });
    return NextResponse.json(adaptPaymentOffer(data));
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    const data = await adminPut<Record<string, unknown>>(`/payment-config/payment-offers/${id}`, body, { requiredRole: "ADMIN" });
    return NextResponse.json(adaptPaymentOffer(data));
  } catch (error) {
    if (error instanceof AdminAuthError) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    if (error instanceof AdminForbiddenError) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await adminDelete(`/payment-config/payment-offers/${id}`, { requiredRole: "ADMIN" });
