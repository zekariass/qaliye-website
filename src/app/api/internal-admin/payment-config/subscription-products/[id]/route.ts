...
export async function GET(
  ...
  try {
    ...
    const data = await adminGet(`/payment-config/subscription-products/${id}`, { requiredRole: "ADMIN" });
...
export async function PUT(
  ...
  try {
    ...
    const data = await adminPut(`/payment-config/subscription-products/${id}`, body, { requiredRole: "ADMIN" });
    return NextResponse.json(data);
  } catch (error) {
    if (error instanceof AdminAuthError) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    if (error instanceof AdminForbiddenError) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await adminDelete(`/payment-config/subscription-products/${id}`, { requiredRole: "ADMIN" });
