...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await adminGet("/payment-config/payment-methods", { requiredRole: "ADMIN", params });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await adminPost("/payment-config/payment-methods", body, { requiredRole: "ADMIN" });
