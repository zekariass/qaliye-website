...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await adminGet("/payment-config/consumable-products", { requiredRole: "ADMIN", params });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await adminPost("/payment-config/consumable-products", body, { requiredRole: "ADMIN" });
