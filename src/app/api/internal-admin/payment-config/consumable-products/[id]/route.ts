...
export async function GET(
  ...
  try {
    ...
    const data = await adminGet(`/payment-config/consumable-products/${id}`, { requiredRole: "ADMIN" });
...
export async function PUT(
  ...
  try {
    ...
    const data = await adminPut(`/payment-config/consumable-products/${id}`, body, { requiredRole: "ADMIN" });
    return NextResponse.json(data);
  } catch (error) {
    if (error instanceof AdminAuthError) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    if (error instanceof AdminForbiddenError) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
