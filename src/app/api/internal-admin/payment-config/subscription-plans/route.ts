...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await adminGet("/payment-config/subscription-plans", { requiredRole: "ADMIN", params });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await adminPost("/payment-config/subscription-plans", body, { requiredRole: "ADMIN" });
